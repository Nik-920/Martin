...
def main():
    ...
    # 2) Filtrar por provincia Huallaga
    df = df[df['PROVINCIA'].str.upper().fillna('') == 'HUALLAGA']

    # 3) Convertir FECHA y extraer día
    df['FECHA'] = pd.to_datetime(df['FECHA'], format='%Y%m%d', errors='coerce')
    df['DIA'] = df['FECHA'].dt.day

    # 4) Convertir HORA_INFRACCION y calcular segundos
    ...
    ax.set_title('Agrupamiento DBSCAN de Infracciones por SEGUNDOS (Provincia: Huallaga)')
